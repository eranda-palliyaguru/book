...
<div class="box-body">
   ...
   <body>
<html>
<?php 		   $result = $db->prepare("SELECT * FROM sub_cat");
		           $result->bindParam(':userid', $res);
		           $result->execute();
		for($i=0; $row = $result->fetch(); $i++){ echo $row['name']; }
	...
	?>
		<option value="<?php echo $row['id'];?>"><?php echo $row['name']; ?> විද්‍යා දදාති විනයං</option>
	<?php
				}
			?>
		</select>
